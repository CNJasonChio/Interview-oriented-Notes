...
由于指定了 `launch::deferred` 策略，因此调用 `async()` 函数并不会马上创建线程并执行任务，必须使用 `future` 类对象调用 `get()` 或者 `wait()` 时才能创建线程（此处一定要注意调用 wait_for () 函数是不行的）。通过示例程序可以推导出在主线程休眠 5 秒后，子线程才能被创建并执行，打印出的结果亦如此。
