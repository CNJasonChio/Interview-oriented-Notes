...
- [ ] [C++和C的区别](#C++和C的区别)
- [ ] [C++和Java的区别](#C++和Java的区别)
- [x] [数组越界如何检查、解决](#数组越界如何检查、解决)
- [x] [new和malloc的区别](#new和malloc的区别)
- [x] [空指针NULL和nullptr的区别](#空指针NULL和nullptr的区别)
- [x] [类的默认成员函数有哪些](#类的默认成员函数有哪些)
- [x] [一个空类占用的内存大小是多少](#一个空类占用的内存大小是多少)
- [x] [类的大小如何计算](#类的大小如何计算)
- [x] [指针的内存大小](#指针的内存大小)
- [ ] [一些关键字](#一些关键字)
- [ ] [通过const成员函数实现非const成员函数](#通过const成员函数实现非const成员函数)
- [x] [const对象、非const对象、const成员函数与非const成员函数的相互调用](#const对象、非const对象、const成员函数与非const成员函数的相互调用)
- [x] [指针和引用](#指针和引用)
- [x] [memcpy和memmove以及memccpy的区别](#memcpy和memmove以及memccpy的区别)
- [x] [内存泄漏](#内存泄漏)
- [x] [内存对齐](#内存对齐)
- [x] [如何判断两个float相等](#如何判断两个float相等)
- [ ] [多线程](#多线程)
- [x] [i++和++i是不是原子操作](#i++和++i是不是原子操作)
- [x] [实现一个不能被继承的类或只能在堆上或栈上生对象的类或不能被拷贝的类或只能定义一个对象的类](#实现一个不能被继承的类或只能在堆上或栈上生对象的类或不能被拷贝的类或只能定义一个对象的类)
- [ ] [静态和非静态成员函数](#静态和非静态成员函数)
- [ ] [结构体与联合体的区别](#结构体与联合体的区别)
- [x] [C++程序编译到运行经历了什么](#C++程序编译到运行经历了什么)
- [ ] [C++内存中的堆栈，堆和栈有具体是什么，有什么区别](#C++内存中的堆栈，堆和栈有具体是什么，有什么区别)
- [ ] [C++野指针、悬挂指针、空指针](#C++野指针、悬挂指针、空指针)
- [ ] [初始化列表](#初始化列表)
- [ ] [C++异常机制](#C++异常机制)
- [ ] [C++自由存储区](#C++自由存储区)
- [ ] [C++对象的内存布局](#C++对象的内存布局)
...
- [ ] [虚函数、纯虚函数、虚函数表以及它们的应用](#虚函数、纯虚函数、虚函数表以及它们的应用)
- [ ] [为什么构造函数不能虚拟化](#为什么构造函数不能虚拟化)
- [ ] [虚析构，析构函数合适可以定义为虚函数](#虚析构，析构函数合适可以定义为虚函数)
- [ ] [析构函数可否抛出异常](#析构函数可否抛出异常)
- [ ] [智能指针](#智能指针)
- [ ] [lambda表达式](#lambda表达式)
- [ ] [类型转换](#类型转换)
- [ ] [多态如何实现](#多态如何实现)
- [ ] [重载的底层实现](#重载的底层实现)
- [ ] [左值和右值](#左值和右值)
- [ ] [C++11的新特性有哪些](#C++11的新特性有哪些)
- [ ] [深拷贝和浅拷贝](#深拷贝和浅拷贝)

### **STL**

- [ ] [常用容器，vector和list区别，resize和reserve区别，erase底层](#常用容器，vector和list区别，resize和reserve区别，erase底层)
- [ ] [STL有哪些组件](#STL有哪些组件)
- [ ] [map和unorder_map有哪些区别,map为什么是log n的查询和修改时间](#map和unorder_map有哪些区别,map为什么是log n的查询和修改时间)
- [ ] [map中红黑树最长路径和最短路径的差值](#map中红黑树最长路径和最短路径的差值)
- [ ] [哈希表如何实现、哈希冲突](#哈希表如何实现、哈希冲突)
- [ ] [模板函数](#模板函数)
- [ ] [vector内存管理](#vector内存管理)
- [ ] [STL中Set中存储一个类对象，如何设计？](#STL中Set中存储一个类对象，如何设计？)
- [ ] [双向queue的实现原理](#双向queue的实现原理)


## 基本概念

### [C++和C的区别](https://github.com/CNJasonChio/Interview-oriented-Notes/blob/master/2.%20C%2B%2B/1.%20%E9%9D%A2%E7%BB%8F%E9%97%AE%E9%A2%98%E6%B1%87%E6%80%BB/1.%20C%2B%2B%E5%92%8CC%E4%BB%A5%E5%8F%8AJava%E7%9A%84%E5%8C%BA%E5%88%AB.md)

### [C++和Java的区别](https://github.com/CNJasonChio/Interview-oriented-Notes/blob/master/2.%20C%2B%2B/1.%20%E9%9D%A2%E7%BB%8F%E9%97%AE%E9%A2%98%E6%B1%87%E6%80%BB/1.%20C%2B%2B%E5%92%8CC%E4%BB%A5%E5%8F%8AJava%E7%9A%84%E5%8C%BA%E5%88%AB.md)
...
### [new和malloc的区别](https://github.com/CNJasonChio/Interview-oriented-Notes/blob/master/2.%20C%2B%2B/1.%20%E9%9D%A2%E7%BB%8F%E9%97%AE%E9%A2%98%E6%B1%87%E6%80%BB/4.%20new%E5%92%8Cmalloc%E7%9A%84%E5%8C%BA%E5%88%AB.md)
...
### [内存对齐](https://github.com/CNJasonChio/Interview-oriented-Notes/blob/master/2.%20C%2B%2B/1.%20%E9%9D%A2%E7%BB%8F%E9%97%AE%E9%A2%98%E6%B1%87%E6%80%BB/16.%20%E5%86%85%E5%AD%98%E5%AF%B9%E9%BD%90.md)
